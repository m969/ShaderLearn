Shader "MagicFire/BasicDiffuse"
{
	...
	SubShader
	{
		...
		////基础漫反射光照模型
		//inline float4 LightingBasicDiffuse(SurfaceOutput s, fixed3 lightDir/*平行光的位置即光的方向*/, fixed atten)
		//{
		//	//计算漫反射强度
		//	float diffLight = max(0.0, (dot(s.Normal, lightDir)));
		//	float4 col;
		//	col.rgb = s.Albedo * _LightColor0.rgb * (diffLight * atten * 0.5);
		//	col.a = s.Alpha;
		//	return col;
		//}
